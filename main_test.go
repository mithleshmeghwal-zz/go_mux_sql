...
func TestCreateUser(t *testing.T) {
	...
	if m["age"] != 30.0 {
		t.Errorf("Expected user age to be '30'. Got '%v'",
			m["age"])
	}
	if m["id"] != 1.0 {
		t.Errorf("Expected user ID to be '1'. Got '%v'",
			m["age"])
	}
}
...
func TestUpdateUser(t *testing.T) {
	...
	if m["id"] != originalUser["id"] {
		t.Errorf("Expected the id to remain the same (%v). Got %v",
			originalUser["id"], m["id"])
	}

	if m["name"] == originalUser["name"] {
		t.Errorf("Expected the name to change from '%v' to '%v'. Got '%v'",
			originalUser["name"], m["name"], m["name"])
	}

	if m["age"] == originalUser["age"] {
		t.Errorf("Expected the age to change from '%v' to '%v'. Got '%v'",
			originalUser["age"], m["age"], m["age"])
	}
}
